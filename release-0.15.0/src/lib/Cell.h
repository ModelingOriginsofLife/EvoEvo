...
public:
  ...
  Cell( Parameters* parameters, gzFile backup_file );
  Cell( const Cell& cell );
  Cell( Cell& parent, unsigned long long int new_id, size_t x, size_t y, size_t time );
  ...
  void get_pumped_and_produced_metabolites( std::vector<int>& pumped, std::vector<int>& produced );
 ...
 * \return   \e double
 */
inline double Cell::get_km_mutation_size( void ) const
{
  return _mutation_rates[KM_MUTATION_SIZE];
}
...
{
  return _mutation_rates[BINDING_SIZE_TAG_MUTATION_SIZE];
}
 ...
 * \return   \e void
 */
inline void Cell::set_km_mutation_size( double size )
{
  assert(size >= 0.0);
  _mutation_rates[KM_MUTATION_SIZE] = size;
}
...
{
  assert(size >= 0.0);
  _mutation_rates[BINDING_SIZE_TAG_MUTATION_SIZE] = size;
}
