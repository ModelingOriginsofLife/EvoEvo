...
public:
  ...
  inline double         get_cumulated_error( void );
  ...
  static int ODE_system( double t, const double y[], double dydt[], void* parameters );
...
protected:
  ...
  void add_metabolic_reaction( reaction_type type, int s, int p, double km, double kcat, double delta_g, size_t e );
  ...
  double              _cumulated_error; /*!< Cumulated error        */
...
/**
 * \brief    Get cumulated error
 * \details  --
 * \param    void
 * \return   \e double
 */
inline double ODE::get_cumulated_error( void )
{
