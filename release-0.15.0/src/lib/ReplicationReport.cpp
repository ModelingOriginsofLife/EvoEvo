...
ReplicationReport::ReplicationReport( gzFile backup_file )
{
  ...
  gzread( backup_file, &_mean_km_mutation_size,   sizeof(_mean_km_mutation_size) );
...
ReplicationReport::ReplicationReport( const ReplicationReport& report )
{
  ...
  _mean_km_mutation_size   = report._mean_km_mutation_size;
...
void ReplicationReport::add_mutation_event( MutationEvent* event )
{
  ...
  {
    pearl* dX = event->get_mutation_vector()->get_dX();
    _nb_point_mutations++;
  ...
  /* 2.2) If mutation event is a point mutation */
...
void ReplicationReport::compute_mean( void )
{
  if (_nb_E_point_mutations > 0)
  {
    ...
    _mean_km_mutation_size   /= _nb_E_point_mutations;
...
void ReplicationReport::save( gzFile backup_file )
{
  ...
  gzwrite( backup_file, &_mean_km_mutation_size,   sizeof(_mean_km_mutation_size) );
