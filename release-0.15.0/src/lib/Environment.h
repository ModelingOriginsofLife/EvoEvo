...
public:
  ...
  inline size_t       get_species_lists_size( void ) const;
  ...
  inline void increase_species_lists_size( size_t size );
  inline void decrease_species_lists_size( size_t size );
   ...
   * PUBLIC METHODS
   *----------------------------*/
  void update_environment_amount( void );
...
protected:
  ...
  size_t        _species_lists_size; /*!< Species list size           */
...
inline double Environment::get( size_t x, size_t y, int tag )
{
  ...
  assert(y < _height);
  assert(tag > 0);
  if (tag > (int)_species_lists_size)
  {
    return 0.0;
  }
 ...
 * \return   \e size_t
 */
inline size_t Environment::get_species_lists_size( void ) const
{
  return _species_lists_size;
}
...
inline void Environment::set( size_t x, size_t y, int tag, double concentration )
{
  ...
  assert(y < _height);
  assert(tag > 0);
  if (_species_lists_size < (size_t)tag)
  {
    increase_species_lists_size((size_t)tag);
  }
...
/**
 * \brief    Increase species lists size in the whole environment
 * \details  --
 * \param    size_t size
 * \return   \e void
 */
inline void Environment::increase_species_lists_size( size_t size )
{
  if (_species_lists_size < size)
  {
    for (size_t i = 0; i < _width*_height; i++)
    {
      _grid[i]->increase_size(size);
    }
    _X->increase_size(size);
  }
  _species_lists_size = size;
}

/**
 * \brief    Decrease species lists size in the whole environment
 * \details  --
 * \param    size_t size
 * \return   \e void
 */
inline void Environment::decrease_species_lists_size( size_t size )
{
  if (size < _species_lists_size)
  {
    for (size_t i = 0; i < _width*_height; i++)
    {
      _grid[i]->decrease_size(size);
    }
    _X->decrease_size(size);
  }
  _species_lists_size = size;
}
