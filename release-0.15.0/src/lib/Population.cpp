...
Population::Population( Parameters* parameters, gzFile backup_file )
{
  ...
  for (size_t i = 0; i < _width*_height; i++)
  {
    _grid[i] = new Cell(_parameters, backup_file);
  }
 ...
 * \details  --
 * \param    const Population& population
 * \return   \e void
 */
Population::Population( const Population& population )
{
  _parameters = population._parameters;
  _width      = population._width;
  _height     = population._height;
  _grid       = new Cell*[_width*_height];
  for (size_t i = 0; i < _width*_height; i++)
  {
    _grid[i] = new Cell(*population._grid[i]);
  }
  _current_id      = population._current_id;
  _best_id         = population._best_id;
  _best_pos        = population._best_pos;
  _time            = population._time;
  _population_size = population._population_size;
  _previous_size   = population._previous_size;
  _growth_rate     = population._growth_rate;
}
