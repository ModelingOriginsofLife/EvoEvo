...
Cell::Cell( Parameters* parameters )
{
  ...
  _genome             = new Genome(_parameters, _replication_report);
 ...
 * \return   \e void
 */
Cell::Cell( Parameters* parameters, gzFile backup_file )
{
  /*------------------------------------------------------------------ simulation parameters */
  ...
  _genome             = new Genome(_parameters, _replication_report, backup_file);
  _inherited_proteins = NULL;
  if (_parameters->get_enzymatic_inheritance())
  {
  ...
  gzread( backup_file, &_mutation_rates[KM_MUTATION_SIZE],                       sizeof(_mutation_rates[KM_MUTATION_SIZE]) );
  gzread( backup_file, &_mutation_rates[KCAT_MUTATION_SIZE],                     sizeof(_mutation_rates[KCAT_MUTATION_SIZE]) );
  gzread( backup_file, &_mutation_rates[BINDING_SIZE_TAG_MUTATION_SIZE],         sizeof(_mutation_rates[BINDING_SIZE_TAG_MUTATION_SIZE]) );
...
Cell::Cell( const Cell& cell )
{
  ...
  _parameters = cell._parameters;
  
  /*------------------------------------------------------------------ main cell classes */
  
  _replication_report = new ReplicationReport(*cell._replication_report);
  _genome             = new Genome(*cell._genome, _replication_report);
  _inherited_proteins = NULL;
  if (_parameters->get_enzymatic_inheritance())
  {
    _inherited_proteins = new InheritedProteins(*cell._inherited_proteins);
  }
  _inherited_species_list = new SpeciesList(*cell._inherited_species_list);
  _species_list           = new SpeciesList(*cell._species_list);
  ...
  _id                  = cell._id;
  _parent_id           = cell._parent_id;
  _generation          = cell._generation;
  _energy              = cell._energy;
  _active              = cell._active;
  _alive               = cell._alive;
  _x                   = cell._x;
  _y                   = cell._y;
  _score               = cell._score;
  _number_of_updates   = cell._number_of_updates;
  _number_of_divisions = cell._number_of_divisions;
  _tagged              = cell._tagged;
  
  /*------------------------------------------------------------------ mutation rates */
  
  _mutation_rates = new double[NUMBER_OF_MUTATION_RATES];
  _mutation_rates[POINT_MUTATION_RATE]                    = cell._mutation_rates[POINT_MUTATION_RATE];
  _mutation_rates[DUPLICATION_RATE]                       = cell._mutation_rates[DUPLICATION_RATE];
  _mutation_rates[DELETION_RATE]                          = cell._mutation_rates[DELETION_RATE];
  _mutation_rates[TRANSLOCATION_RATE]                     = cell._mutation_rates[TRANSLOCATION_RATE];
  _mutation_rates[INVERSION_RATE]                         = cell._mutation_rates[INVERSION_RATE];
  _mutation_rates[TRANSITION_RATE]                        = cell._mutation_rates[TRANSITION_RATE];
  _mutation_rates[SUBSTRATE_TAG_MUTATION_SIZE]            = cell._mutation_rates[SUBSTRATE_TAG_MUTATION_SIZE];
  _mutation_rates[PRODUCT_TAG_MUTATION_SIZE]              = cell._mutation_rates[PRODUCT_TAG_MUTATION_SIZE];
  _mutation_rates[KM_MUTATION_SIZE]                       = cell._mutation_rates[KM_MUTATION_SIZE];
  _mutation_rates[KCAT_MUTATION_SIZE]                     = cell._mutation_rates[KCAT_MUTATION_SIZE];
  _mutation_rates[BINDING_SIZE_TAG_MUTATION_SIZE]         = cell._mutation_rates[BINDING_SIZE_TAG_MUTATION_SIZE];
  _mutation_rates[CO_ENZYME_TAG_MUTATION_SIZE]            = cell._mutation_rates[CO_ENZYME_TAG_MUTATION_SIZE];
  _mutation_rates[TRANSCRIPTION_FACTOR_TAG_MUTATION_SIZE] = cell._mutation_rates[TRANSCRIPTION_FACTOR_TAG_MUTATION_SIZE];
  _mutation_rates[BASAL_EXPRESSION_LEVEL_MUTATION_SIZE]   = cell._mutation_rates[BASAL_EXPRESSION_LEVEL_MUTATION_SIZE];
  
  /*------------------------------------------------------------------ time variables */
  
  _birth_time = cell._birth_time;
  _death_time = cell._death_time;
  _lifespan   = cell._lifespan;
  
  /*------------------------------------------------------------------ global phenotypic variables */
  
  _toxicity                   = cell._toxicity;
  _inherited_TF_amount        = cell._inherited_TF_amount;
  _inherited_E_amount         = cell._inherited_E_amount;
  _TF_amount                  = cell._TF_amount;
  _E_amount                   = cell._E_amount;
  _min_metabolic_amount       = cell._min_metabolic_amount;
  _max_metabolic_amount       = cell._max_metabolic_amount;
  _metabolic_uptake           = cell._metabolic_uptake;
  _metabolic_release          = cell._metabolic_release;
  _previous_metabolic_amount  = cell._previous_metabolic_amount;
  _min_energy                 = cell._min_energy;
  _mean_energy                = cell._mean_energy;
  _max_energy                 = cell._max_energy;
  _min_score                  = cell._min_score;
  _mean_score                 = cell._mean_score;
  _max_score                  = cell._max_score;
  _metabolic_growth_rate      = cell._metabolic_growth_rate;
  _diff_metabolic_growth_rate = cell._diff_metabolic_growth_rate;
  _grn_nb_nodes               = cell._grn_nb_nodes;
  _grn_nb_edges               = cell._grn_nb_edges;
  _metabolic_nb_nodes         = cell._metabolic_nb_nodes;
  _metabolic_nb_edges         = cell._metabolic_nb_edges;
  _inflowing_pumps.clear();
  _outflowing_pumps.clear();
  for (size_t i = 0; i < cell._inflowing_pumps.size(); i++)
  {
    _inflowing_pumps.push_back(cell._inflowing_pumps[i]);
  }
  for (size_t i = 0; i < cell._outflowing_pumps.size(); i++)
  {
    _outflowing_pumps.push_back(cell._outflowing_pumps[i]);
  }
  _trophic_group = cell._trophic_group;
  _trophic_level = cell._trophic_level;
  
  /*------------------------------------------------------------------ cell color */
  
  _red_color   = cell._red_color;
  _green_color = cell._green_color;
  _blue_color  = cell._blue_color;
}
 ...
 * \return   \e void
 */
Cell::Cell( Cell& parent, unsigned long long int new_id, size_t x, size_t y, size_t time)
{
  /*------------------------------------------------------------------ simulation parameters */
  ...
  /* copy genome            */
  /*------------------------*/
  _genome = new Genome(*parent._genome, _replication_report);
  ...
  if (_parameters->get_enzymatic_inheritance())
  {
    ...
    {
      if (_inherited_proteins->get_pearl(i)->type == TRANSCRIPTION_FACTOR)
      {
        _inherited_TF_amount += _inherited_proteins->get_concentration_vector()[i];
      }
      else if (_inherited_proteins->get_pearl(i)->type == ENZYME)
      {
  ...
  _inherited_species_list->reset(_parameters->get_metabolic_inheritance());
  _species_list->reset(_parameters->get_metabolic_inheritance());
  parent._species_list->reset(_parameters->get_metabolic_inheritance());
  ...
  _mutation_rates = new double[NUMBER_OF_MUTATION_RATES];
  _mutation_rates[POINT_MUTATION_RATE]                    = parent._mutation_rates[POINT_MUTATION_RATE];
  _mutation_rates[DUPLICATION_RATE]                       = parent._mutation_rates[DUPLICATION_RATE];
  _mutation_rates[DELETION_RATE]                          = parent._mutation_rates[DELETION_RATE];
  _mutation_rates[TRANSLOCATION_RATE]                     = parent._mutation_rates[TRANSLOCATION_RATE];
  _mutation_rates[INVERSION_RATE]                         = parent._mutation_rates[INVERSION_RATE];
  _mutation_rates[TRANSITION_RATE]                        = parent._mutation_rates[TRANSITION_RATE];
  _mutation_rates[SUBSTRATE_TAG_MUTATION_SIZE]            = parent._mutation_rates[SUBSTRATE_TAG_MUTATION_SIZE];
  _mutation_rates[PRODUCT_TAG_MUTATION_SIZE]              = parent._mutation_rates[PRODUCT_TAG_MUTATION_SIZE];
  _mutation_rates[KM_MUTATION_SIZE]                       = parent._mutation_rates[KM_MUTATION_SIZE];
  _mutation_rates[KCAT_MUTATION_SIZE]                     = parent._mutation_rates[KCAT_MUTATION_SIZE];
  _mutation_rates[BINDING_SIZE_TAG_MUTATION_SIZE]         = parent._mutation_rates[BINDING_SIZE_TAG_MUTATION_SIZE];
  _mutation_rates[CO_ENZYME_TAG_MUTATION_SIZE]            = parent._mutation_rates[CO_ENZYME_TAG_MUTATION_SIZE];
  _mutation_rates[TRANSCRIPTION_FACTOR_TAG_MUTATION_SIZE] = parent._mutation_rates[TRANSCRIPTION_FACTOR_TAG_MUTATION_SIZE];
  _mutation_rates[BASAL_EXPRESSION_LEVEL_MUTATION_SIZE]   = parent._mutation_rates[BASAL_EXPRESSION_LEVEL_MUTATION_SIZE];
...
void Cell::load_genome_in_inherited_proteins( void )
{
  ...
  str         = _genome->get_string_of_pearl()->x;
...
void Cell::synchronize_state_vectors( Environment* environment )
{
  ...
  /*------------------------------------------------------------------*/
  if (_species_list->get_size() < environment->get_species_lists_size())
  {
    _inherited_species_list->increase_size(environment->get_species_lists_size());
    _species_list->increase_size(environment->get_species_lists_size());
  }
  else if (environment->get_species_lists_size() < _species_list->get_size())
  {
    environment->increase_species_lists_size(_species_list->get_size());
  }
}
...
void Cell::update( size_t time )
{
  ...
  for (size_t i = 0; i < _genome->get_size(); i++)
  {
    if (_genome->get_pearl(i)->type == TRANSCRIPTION_FACTOR)
    {
      _TF_amount += _genome->get_concentration_vector()[i];
    }
    else if (_genome->get_pearl(i)->type == ENZYME)
    {
...
void Cell::compute_score( void )
{
  ...
  /*-----------------------------------------------*/
  if (_parameters->get_energy_constraints() && _energy <= 0.0)
  {
    _score = 0.0;
    return;
  }
  ...
  /*-----------------------------------------------*/
  else if (_parameters->get_energy_constraints() && _energy > _parameters->get_energy_toxicity_threshold())
  {
    if (_parameters->get_variable_lifespan())
    {
      _toxicity += _energy-_parameters->get_energy_toxicity_threshold();
    }
    else
    {
      _score = 0.0;
  ...
  /*-----------------------------------------------*/
  else if (!_parameters->get_energy_constraints())
  {
    _energy = 0.0;
  }
  ...
  if (_parameters->get_score_scheme() == ESSENTIAL_METABOLITES_SUM)
  {
    ...
    /*----------------------------------------*/
    /* B.2) if lifespan is a Poisson process, */
    ...
    /*----------------------------------------*/
    /* B.3) if lifespan is a Gompertz law,    */
    /*      add threshold quadratic error to  */
    /*      cell's toxicity                   */
    /*----------------------------------------*/
    else if (_parameters->get_variable_lifespan())
    {
      if (toxicity_count > 0)
      {
        _toxicity += toxicity;
  ...
  else if (_parameters->get_score_scheme() == ESSENTIAL_METABOLITES_SUM_MINUS_DEVIATION)
  {
    ...
    /*-------------------------------------------*/
    /* C.3) if lifespan is a Poisson process,    */
    /*      apply toxicity threshold             */
    /*-------------------------------------------*/
    if (!_parameters->get_variable_lifespan() && toxicity_threshold_reached)
    {
      _score = 0.0;
    }
    /*-------------------------------------------*/
    /* C.4) if lifespan is a Gompertz law, add   */
    /*      threshold quadratic error to cell's  */
    /*      toxicity                             */
    /*-------------------------------------------*/
    else if (_parameters->get_variable_lifespan())
    {
...
void Cell::save( gzFile backup_file )
{
  ...
  gzwrite( backup_file, &_mutation_rates[KM_MUTATION_SIZE],                       sizeof(_mutation_rates[KM_MUTATION_SIZE]) );
  gzwrite( backup_file, &_mutation_rates[KCAT_MUTATION_SIZE],                     sizeof(_mutation_rates[KCAT_MUTATION_SIZE]) );
  gzwrite( backup_file, &_mutation_rates[BINDING_SIZE_TAG_MUTATION_SIZE],         sizeof(_mutation_rates[BINDING_SIZE_TAG_MUTATION_SIZE]) );
...
void Cell::write_genome( std::ofstream& filestream )
{
  ...
  for (size_t i = 0; i < _genome->get_size(); i++)
  {
    ...
    filestream << p->s << " " << p->p << " " << p->km << " " << p->kcat << " ";
    
    /*------------------------------------------------------------------ Transcription factor type (TF) attributes */
    
    filestream << p->BS_tag << " " << p->coE_tag << " ";
    filestream << p->free_activity << " " << p->bound_activity << " " << p->window << " ";
...
void Cell::write_inherited_proteins( std::ofstream& filestream )
{
  ...
  for (size_t i = 0; i < _inherited_proteins->get_size(); i++)
  {
    pearl* p = _inherited_proteins->get_pearl(i);
...
void Cell::write_genetic_regulation_network( std::ofstream& nodeslist, std::ofstream& edgeslist )
{
  ...
  for (size_t region = 0; region < list->grn_N; region++)
  {
    ...
    for (size_t pos = 0; pos < Nenhancer[region]; pos++)
    {
      ...
      for (size_t gene_index = 0; gene_index < Ngenes[region]; gene_index++)
      {
        ...
        pearl_type TGene = _genome->get_pearl(regulated_genes[regulated_genes_index+gene_index])->type;
        /* Add regulated gene */
  ...
  for (std::unordered_map<size_t, grn_node_type>::iterator it = nodes_map.begin(); it != nodes_map.end(); ++it)
  {
    ...
    {
      pearl* p = _genome->get_pearl(it->first);
      nodeslist << it->first << " " << _genome->get_concentration_vector()[it->first] << " E " << p->s << " " << p->p << " " << p->km << " " << p->kcat << " 0\n";
    }
    /*------------------------------------*/
...
void Cell::write_metabolic_network( std::ofstream& nodeslist, std::ofstream& edgeslist )
{
  ...
  for (size_t i = 0; i < list->metabolic_N; i++)
  {
    ...
    /* 3.2) if reaction is metabolic catalytic activity ------*/
    if (type == CATALYTIC_ACTIVITY)
    {
    ...
    /* 3.3) if reaction is metabolic inflowing pump activity -*/
    else if (type == INFLOWING_PUMP_ACTIVITY)
    {
      if (nodes_map.find(-s) == nodes_map.end())
      {
        nodes_map[-s] = 1;
    ...
    /* 3.4) if reaction is metabolic outflowing pump activity */
...
/**
 * \brief    Write the current ODE system
...
void Cell::write_current_ODE_system( void )
{
  ...
  file << "-----------------\n";
  file << "For enzyme pearls: if kcat < 0.0, the reaction is reverted (e.g. 4->4 with kcat > 0.0 is an uptake pump ; the same reaction with kcat < 0.0 is a release pump).\n";
  file << "\n";
  for (size_t i = 0; i < _genome->get_size(); i++)
  {
    pearl* p = _genome->get_pearl(i);
    ...
    {
      file << "y[" << i << "] (ENZYME) s=" << p->s << ", p=" << p->p << ", km=" << p->km << ", kcat=" << p->kcat << "\n";
    }
    else if (p->type == TRANSCRIPTION_FACTOR)
    {
      file << "y[" << i << "] (TRANSCRIPTION_FACTOR) BS_tag=" << p->BS_tag << ", coE=" << p->coE_tag << ", bound_activity=" << p->bound_activity << ", free_activity=" << p->free_activity << "\n";
    }
    else if (p->type == BINDING_SITE)
    {
      file << "y[" << i << "] (BINDING_SITE) TF_tag=" << p->TF_tag << "\n";
    }
    else if (p->type == PROMOTER)
    {
      file << "y[" << i << "] (PROMOTER) basal_expression_level=" << p->basal_expression_level << "\n";
    }
  }
  file << "\n";
 ...
 * \return   \e void
 */
void Cell::get_pumped_and_produced_metabolites( std::vector<int>& pumped, std::vector<int>& produced )
{
  pumped.clear();
  produced.clear();
  ...
  for (size_t i = 0; i < list->metabolic_N; i++)
  {
    ...
    else if (type == CATALYTIC_ACTIVITY)
    {
