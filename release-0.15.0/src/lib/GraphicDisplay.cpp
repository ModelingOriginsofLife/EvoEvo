 ...
 * \details  --
 * \param    const GraphicDisplay& graphicDisplay 
 * \return   \e void
 */
GraphicDisplay::GraphicDisplay( const GraphicDisplay& graphicDisplay )
{
  /*------------------------------------------------------------------ main parameters */
  
  _parameters  = graphicDisplay._parameters;
  _simulation = graphicDisplay._simulation;
  _pop_window = graphicDisplay._pop_window;
  _env_window = graphicDisplay._env_window;
  ...
  _font_path = graphicDisplay._font_path;
  if (!_font.loadFromFile(_font_path))
  {
    printf("ERROR: Font not found in GraphicDisplay copy constructor.\n");
    exit(EXIT_FAILURE);
  }
  _text_up.setFont(_font);
  _text_up.setCharacterSize(10);
  ...
  _color_scheme = graphicDisplay._color_scheme;
}
...
{
  /*---------------------------------------*/
  /* 1) evaluate population render window  */
  /*---------------------------------------*/
