...
/**
 * \file      trophic_groups_history.cpp
 ...
 * \brief     Rebuild the history of trophic groups
 */
...
const std::string EXECUTABLE_NAME = "build/bin/trophic_groups_history";
...
int main( int argc, char const** argv )
{
  ...
  /*---------------------------------------*/
  /* 3) recover the trophic groups history */
  /*---------------------------------------*/
  ...
  while (current_step <= backup_end)
  {
    ...
    for (it = profiles.begin(); it != profiles.end(); ++it)
    {
      ...
      while (group != NULL)
      {
        std::string profile = group->get_uptake_profile();
        if (profile.size() < L)
        {
          std::string seq;
...
void printUsage( void )
{
  ...
  std::cout << "***************************************************************************\n";
  std::cout << "Usage: trophic_groups_history -h or --help\n";
  std::cout << "   or: trophic_groups_history [-b backup-time]\n";
...
size_t get_uptake_profile_max_length( size_t backup_start, size_t backup_end, size_t backup_step )
{
  ...
  while (current_step <= backup_end)
  {
    ...
    Simulation* simulation = new Simulation(parameters, current_step, false);
    simulation->initialize(FROM_BACKUP);
    size_t L = simulation->get_trophic_network()->get_first_group()->get_uptake_profile().size();
    if (max_length < L)
    {
      max_length = L;
    }
