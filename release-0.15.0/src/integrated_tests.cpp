...
const std::string EXECUTABLE_NAME  = "build/bin/integrated_tests";
...
int main( int argc, char const** argv )
{
  ...
  {
    load_successful = parameters1->load_parameters_from_file(optional_filename);
  }
  else
  {
    load_successful = parameters1->load_parameters_from_file(DEFAULT_FILENAME);
  }
  if (!load_successful)
  {
    printUsage();
  ...
  Parameters* parameters2 = new Parameters(*parameters1);
...
void printUsage( void )
{
  ...
  std::cout << "   or: integrated_tests [-f param-file]\n";
  ...
  std::cout << "        specify the number of 500t steps by test (default: 1)\n";
  std::cout << "  -rs, --random-seed\n";
  std::cout << "        specify if the prng seed should be drawn at random for each test\n";
  std::cout << "  -rp, --random-parameters\n";
  std::cout << "        specify if the parameters should be drawn at random for each test\n";
  std::cout << "\n";
}
